...
def retrieveNewsFeed(username, db):
    ...
    for preference in preferences:
        if User.getUser(db, 1, preference):
            ...
            query = "SELECT nf.author, nf.title, nf.url, nf.dateandtime, nf.category from newsfeed nf, users u WHERE nf.category = %s and u." + preference + " = 1 and u.user_id = %s"
            print(query)
            print(user.id)
    ...
    if user:
        for keyword in keywords:
            for word in keyword: #we want to search for the category with the key term each iteration
                ...
                cursor = connection.cursor(pymysql.cursors.DictCursor)
                query2 = "SELECT nf.author, nf.title, nf.url, nf.dateandtime, nf.category from newsfeed nf, users u WHERE nf.category = %s and u.user_id = %s"
